window.onload = function() {
    ...
    function resizeCanvas() {
        ...
        if (isMobileDevice()) {
            keySize = Math.min(canvas.width, canvas.height) / 7; // اندازه کلید بزرگتر برای موبایل
            staffTopMargin = canvas.height / 5; // فاصله بیشتر خط‌ها برای موبایل
            staffSpacing = canvas.height / 7; // فاصله بیشتر خط‌ها برای موبایل
            lineColors = Array(5).fill("#000000"); // رنگ خطوط پررنگ‌تر
            toggleNotesButton.style.fontSize = '36px'; // اندازه دکمه‌ها سه برابر برای موبایل
            showLabelsButton.style.fontSize = '36px'; // اندازه دکمه‌ها سه برابر برای موبایل
        } else {
            keySize = Math.min(canvas.width, canvas.height) / 10; // اندازه کلید دو برابر برای حالت معمولی
            toggleNotesButton.style.fontSize = '12px'; // اندازه دکمه‌ها برای حالت معمولی
            showLabelsButton.style.fontSize = '12px'; // اندازه دکمه‌ها برای حالت معمولی
        ...
        staffPositions = Array.from({ length: 5 }, (_, i) => ({ x: 200 + (isMobileDevice() ? 50 : 0), y: staffTopMargin + i * staffSpacing, width: staffWidth, height: isMobileDevice() ? 10 : 5 })); // ضخیم‌تر شدن خط‌ها در موبایل
        betweenStaffPositions = Array.from({ length: 4 }, (_, i) => ({ x: 200 + (isMobileDevice() ? 50 : 0), y: staffTopMargin + staffSpacing / 2 + i * staffSpacing, width: staffWidth, height: isMobileDevice() ? 10 : 5 })); // ضخیم‌تر شدن خط‌ها در موبایل
    }
    ...
    function drawNotes() {
        ...
        positions.forEach((y, i) => {
            ...
            context.beginPath();
            context.arc(x, y, Math.min(canvas.width, canvas.height) / (isMobileDevice() ? 30 : 40), 0, 2 * Math.PI); // تنظیم اندازه نت‌ها برای موبایل و حالت معمولی
            context.fillStyle = color;
            context.fill();
            context.fillStyle = "#FFFFFF";
            context.font = `${Math.min(canvas.width, canvas.height) / (isMobileDevice() ? 30 : 40)}px Arial`; // تنظیم اندازه فونت نت‌ها
