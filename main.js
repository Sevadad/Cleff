window.onload = function() {
    ...
    function resizeCanvas() {
        ...
        if (isMobileDevice()) {
            keySize = Math.min(canvas.width, canvas.height) / 8; // اندازه کلید بزرگتر برای موبایل
        ...
        staffSpacing = isMobileDevice() ? canvas.height / 8 : canvas.height / 10; // فاصله خط‌ها برای موبایل بیشتر شود
    ...
    function drawNotes() {
        ...
        positions.forEach((y, i) => {
            ...
            context.beginPath();
            context.arc(x, y, Math.min(canvas.width, canvas.height) / (isMobileDevice() ? 30 : 40), 0, 2 * Math.PI); // اندازه نت‌ها برای موبایل بزرگتر
            context.fillStyle = color;
            context.fill();
            context.fillStyle = "#FFFFFF";
            context.font = `${Math.min(canvas.width, canvas.height) / (isMobileDevice() ? 30 : 40)}px Arial`; // اندازه فونت نت‌ها برای موبایل بزرگتر
