window.onload = function() {
    ...
    function drawNotes() {
        ...
        positions.forEach((y, i) => {
            ...
            context.beginPath();
            context.arc(x, y, Math.min(canvas.width, canvas.height) / 40, 0, 2 * Math.PI); // تنظیم اندازه نت‌ها
            context.fillStyle = color;
            context.fill();
            context.fillStyle = "#FFFFFF";
            context.font = `${Math.min(canvas.width, canvas.height) / 40}px Arial`; // تنظیم اندازه فونت نت‌ها
