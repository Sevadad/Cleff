window.onload = function() {
    ...
    function resizeCanvas() {
        ...
        if (isMobileDevice()) {
            keySize = Math.min(canvas.width, canvas.height) / 10; // اندازه کلید کوچک‌تر برای موبایل
            keySpacing = keySize * 1.1; // فاصله کلیدها در حالت موبایل
            staffSpacing = canvas.height / 6; // فاصله خطوط حامل در حالت موبایل
        } else {
            keySize = Math.min(canvas.width, canvas.height) / 8; // دو برابر کردن اندازه کلیدها برای ویندوز
            keySpacing = keySize * 0.8; // کاهش فاصله کلیدها در حالت غیر موبایل
            staffSpacing = canvas.height / 8; // دو برابر کردن فاصله خطوط حامل در حالت غیر موبایل
        }
    ...
    function drawStaffLines() {
        ...
        if (isMobileDevice()) {
            staffPositions.forEach((pos) => {
                context.fillStyle = "#000000";
                context.fillRect(pos.x, pos.y, pos.width, pos.height * 2); // ضخیم‌تر کردن خطوط در حالت موبایل
            });
        }
    }
    ...
    function drawNotes() {
        ...
        positions.forEach((y, i) => {
            ...
            context.beginPath();
            context.arc(x, y, Math.min(canvas.width, canvas.height) / 25, 0, 2 * Math.PI); // تنظیم اندازه نت‌ها
            context.fillStyle = color;
            context.fill();
            context.fillStyle = "#FFFFFF";
            context.font = `${Math.min(canvas.width, canvas.height) / 20}px Arial`; // تنظیم اندازه فونت نت‌ها
    ...
    if (isMobileDevice()) {
        ...
        toggleNotesButton.style.fontSize = '36px'; // تغییر اندازه دکمه‌ها
        showLabelsButton.style.fontSize = '36px'; // تغییر اندازه دکمه‌ها
        toggleKeysButton.style.fontSize = '36px'; // تغییر اندازه دکمه‌ها
    }
